...
# Supprimer les individus DNA 
function deleteDNA(df)
    df = filter(row -> row[:HDA] != 999, df)
end


# Supprimer les individus DNF
function deleteDNF(df)
    df = filter(row -> row[:HDA] != 998, df)
end
